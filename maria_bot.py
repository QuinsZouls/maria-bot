...
def createOrder():
    ...
    for coin in volume:
        ...
        # Check if there balance available
    ...
    for coin in coins_bought:
        if coins_bought[coin] != None:
            ...
            if float(last_price[coin]['price']) > TP or float(last_price[coin]['price']) < SL:
                ...
                # Get step size for apply decimals
                for filt in client.get_symbol_info(coin)['filters']:
                    if filt['filterType'] == 'LOT_SIZE':
                        stepSize = float(filt['stepSize'])
                # Get the exact balance
                currentBalance = client.get_asset_balance(
                    asset=coin.replace(PAIR_WITH, ''))
                precision = int(round(-math.log(stepSize, 10), 0))
                if TESTNET:
                    # create test order before pushing an actual order
                    test_order = client.create_test_order(
                        symbol=coin, side='SELL', type='MARKET', quantity=round(float(currentBalance['free']), precision))
